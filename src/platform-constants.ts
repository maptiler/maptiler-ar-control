/**
 * This source is borrowed from Model-Viewer because the constants are not exposed
 */
...
export const HAS_WEBXR_DEVICE_API =
  navigator.xr != null &&
  (self as SelfWithXR).XRSession != null &&
  navigator.xr.isSessionSupported != null;

export const HAS_WEBXR_HIT_TEST_API =
  HAS_WEBXR_DEVICE_API &&
  (self as SelfWithXR).XRSession &&
  (self as SelfWithXR).XRSession?.requestHitTestSource != null;
...
export const IS_MOBILE = (() => {
  const userAgent =
    navigator.userAgent || navigator.vendor || (self as SelfWithXR).opera;
  let check = false;
...
export const IS_WKWEBVIEW = Boolean(
